...
class DiscreteAutoregressiveFlow(nn.Module):
    ...
    def _initial_call(self, new_inputs, length, **kwargs):
        ...
        **kwargs: Optional keyword arguments to layer.
        ...
        if net.shape[-1] == 2 * self.vocab_size: # if the network outputted both a location and scale.
            ...
            loc = loc[..., 0:1, :] #
            loc = utils.one_hot_argmax(loc, self.temperature).type(inputs.dtype)
            scale = scale[..., 0:1, :]
            scale = utils.one_hot_argmax(scale, self.temperature).type(inputs.dtype)
            inverse_scale = utils.multiplicative_inverse(scale, self.vocab_size) # could be made more efficient by calculating the argmax once and passing it into both functions. 
    ...
    def forward(self, inputs, **kwargs):
        ...
        if net.shape[-1] == 2 * self.vocab_size:
            ...
            raise ValueError('Output of layer does not have compatible dimensions.')
        loc = utils.one_hot_argmax(loc, self.temperature).type(inputs.dtype)
        outputs = utils.one_hot_add(scaled_inputs, loc)
        return outputs
...
class DiscreteBipartiteFlow(nn.Module):
    ...
    def reverse(self, inputs, **kwargs):
        ...
        if layer_outs.shape[-1] == 2 * self.vocab_size: # have a location and scaling parameter
            ...
            inverse_scale = utils.multiplicative_inverse(scale, self.vocab_size)
            shifted_inputs = utils.one_hot_minus(z1, loc)
            x1 = utils.one_hot_multiply(shifted_inputs, inverse_scale)

        elif layer_outs.shape[-1] == self.vocab_size:
            loc = layer_outs
            loc = utils.one_hot_argmax(loc, self.temperature).type( inputs.dtype)
            x1 = utils.one_hot_minus(z1, loc)
        else:
            raise ValueError('Output of layer does not have compatible dimensions.')
        if self.parity:
            x0, x1 = x1, x0
    ...
    def forward(self, inputs, **kwargs):
        ...
        if layer_outs.shape[-1] == 2 * self.vocab_size:
            ...
            scaled_inputs = utils.one_hot_multiply(x1, scale)
        # just outputting loc
        elif layer_outs.shape[-1] == self.vocab_size:
            loc = layer_outs
            scaled_inputs = x1
            ...
            raise ValueError('Output of layer does not have compatible dimensions.')
        loc = utils.one_hot_argmax(loc, self.temperature).type(inputs.dtype)
        z1 = utils.one_hot_add(loc, scaled_inputs)
        if self.parity:
            z0, z1 = z1, z0
